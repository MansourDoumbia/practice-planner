...
const useStore = () => {
  ...
  const [practices, setPractices] = useState(SEED_PRACTICES);
  const [attendance, setAttendance] = useState(SEED_ATTENDANCE);
  const [boats, setBoats] = useState(SEED_BOATS);
  const [lineups, setLineups] = useState(SEED_LINEUPS);
...
const AthleteHousing = ({ user, store, show }) => {
  ...
  const save = () => {
    setUsers(prev => prev.map(u => u.id === user.id ? { ...u, housingLocation: housing, isDriver, carCapacity: Number(cap) } : u));
...
const AthleteReport = ({ user, store, show }) => {
  ...
  const submit = () => {
    if (!missingId) return;
    setReports(prev => [...prev, { id: `rep${Date.now()}`, practiceId, reportedByAthleteId: user.id, missingAthleteId: missingId, notes, timestamp: new Date().toISOString() }]);
...
const SecTeam = ({ store, show }) => {
  ...
  const openEdit = (u) => { setEditing(u); setForm({ ...u }); setShowModal(true); };
  const save = () => {
    ...
    if (editing) {
      ...
      setUsers(prev => [...prev, { ...form, id: `u${Date.now()}`, carCapacity: Number(form.carCapacity) }]);
      show("Athlete added");
    }
    setShowModal(false);
  };
...
const AnnouncementsPage = ({ user, store, show }) => {
  ...
  const send = () => {
    if (!msg.trim()) return;
    setAnnouncements(prev => [...prev, { id: `an${Date.now()}`, senderId: user.id, message: msg, recipientGroup: group, timestamp: new Date().toISOString() }]);
...
const CoachPractices = ({ user, store, show }) => {
  ...
  const create = () => {
    if (!form.date) return;
    const p = { id: `p${Date.now()}`, ...form, createdBy: user.id };
...
const CoachBoats = ({ store, show }) => {
  ...
  const save = () => {
    if (!form.name) return;
    if (editing) setBoats(prev => prev.map(b => b.id === editing.id ? { ...b, ...form } : b));
    else setBoats(prev => [...prev, { ...form, id: `b${Date.now()}`, seatCount: Number(form.seatCount) }]);
